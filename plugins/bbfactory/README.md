...
The CLI exposes the same records through `bb factory <action> --input '<JSON>'`, plus `judge` for an explicitly authorized human attestation. `judge` requires `humanConfirmed:true`, actor, requirement ID, accepted boolean and rationale. These are user attestations, not cryptographic provenance: an API credential holder can submit one. Agents must never invent human authorization.
