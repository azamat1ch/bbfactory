...
const WORKFLOW_PANEL_ACTION_ID = "factory-execution-run";
...
function requireRunId(value: unknown): string | null {
  ...
  return /^wfr_(?:exec_)?[0-9a-f-]+$/i.test(runId) ? runId : null;
}
...
function WorkflowPreviewDirective({
  ...
  if (runId === null) {
    return (
      <EmptyOrError>
        factory-execution-preview requires exactly one valid run attribute, e.g.{" "}
        <code>::factory-execution-preview{'{run="wfr_…"}'}</code>
      </EmptyOrError>
    );
  }
  return (
...
export function registerExecutionUi(app: PluginAppBuilder) {
  app.slots.messageDirective({
    id: "factory-execution-preview",
...
export default definePluginApp(registerExecutionUi);
