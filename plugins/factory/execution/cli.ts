...
function statusSummary(page: WorkflowRunInspectionPage) {
  ...
  return {
    ...
    history: {
      format: "jsonl",
      pageUsage: `bb factory execution history ${run.id} --cursor 0 --limit ${DEFAULT_HISTORY_LIMIT}`,
      fileUsage: `mkdir -p "$BB_THREAD_STORAGE/workflows" && bb factory execution history ${run.id} --cursor 0 --limit ${DEFAULT_HISTORY_LIMIT} > "$BB_THREAD_STORAGE/workflows/${run.id}.jsonl"`,
    },
  };
}
...
const RUN_ID_POSITIONAL = {
  name: "run-id",
  description: "Workflow run ID returned by bb factory execution run",
  required: true,
} as const;
