...
export default async function plugin(bb: BbPluginApi) {
  ...
  const execution = registerExecutionRpc(bb, db, service);
  ...
  const configuration: Parameters<BbPluginApi["agents"]["configure"]>[0] = (
    context,
  ) => {
    ...
    if (context.origin.pluginId === bb.pluginId) {
      return {
        tools: ["bb_factory_result"],
        skills: [],
        instructions:
          "You are starting as a BB workflow worker. Follow the workflow prompt. Your final text IS the return value, not a human-facing message. If the prompt requests structured output, call bb_factory_result exactly once at the end of your response.",
      };
    }
    return { tools: [], skills: [] };
  };
  bb.agents.configure(configuration);
  ...
  return { service, execution, configuration };
}
