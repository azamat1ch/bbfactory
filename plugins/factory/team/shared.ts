...
export const TEAM_INSTRUCTIONS = `Factory connects a conversational spec to native assignments and versioned evidence. Use the factory skill for substantial work and bb factory --help to discover commands. Team preferences are binding delegation constraints unless explicitly overridden by the user for the current task. You MUST read bb factory team get --json before choosing or launching workers, including reviewers and lower-level thread launches; reread after a Team change. Off means do the work directly: do not delegate. Auto allows useful delegation without asking again. Selected means use ONLY the saved provider/model profiles with their configured reasoning and service tier; never silently substitute. If a required profile is unavailable, work directly when practical or explain the blocker and ask before substituting. Direct work remains valid; selecting profiles does not require launching every profile. Do not change saved preferences or the chosen lead without user instruction. Worker completion is not acceptance; record tests, agent inspection or explicit human judgment against the reviewed version.`;
