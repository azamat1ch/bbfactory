...
host. For example, opencode, omp, Grok Build's grok CLI, Hermes' hermes CLI, or
Devin's devin CLI on PATH appears as provider acp-opencode, acp-omp, acp-grok,
acp-hermes-agent, or acp-devin.

bb indexes the native user and project skill roots for Codex, Claude Code, Pi,
Cursor, OpenCode, omp, Grok Build, Hermes Agent, and Devin. This includes compatibility
...
omp, grok, hermes-agent and devin are not reserved, so an entry with one of
those ids replaces the shipped agent. Use args, env, and cwd for the launch, modelCli
