...
`hermes-agent`, `devin`) is not, so an entry with that id REPLACES the shipped
agent.
...
| Devin        | `$XDG_CONFIG_HOME/devin/skills` or `~/.config/devin/skills`, `$XDG_CONFIG_HOME/cognition/skills` or `~/.config/cognition/skills`, `~/.agents/skills`, `~/.claude/skills`, `~/.cursor/skills` | `.devin/skills`, `.cognition/skills`, `.agents/skills`, `.windsurf/skills`, `.cursor/skills`, and `.claude/skills` from the repository root to the current directory |
